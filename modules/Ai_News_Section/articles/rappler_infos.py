...
try:
    ...
    existing_df = pd.DataFrame(columns=["Title", "Date", "URL", "Author", "Source"])  # Add "Source" column
...
for link in rappler_links:
    ...
    if response.status_code == 200:
        ...
        if title not in existing_df["Title"].values:
            ...
            article_data.append([title, date, link, author, "Rappler"])  # Add "Rappler" as the source
        else:
...
new_df = pd.DataFrame(article_data, columns=["Title", "Date", "URL", "Author", "Source"])  # Add "Source" column
